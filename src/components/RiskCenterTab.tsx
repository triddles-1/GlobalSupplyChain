...
export default function RiskCenterTab({ suppliers, settings, currentUser, onUpdateSettings }: RiskCenterTabProps) {
  ...
  return (
    <div className="space-y-6 font-sans">
      {/* Intro Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-white font-display">Mathematical Risk Scoring Model</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 bg-slate-800/30 border border-slate-700/50 rounded-2xl p-6 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800/60 pb-3">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 font-display">
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex justify-between items-center mb-1 text-xs">
                ...
                <span className="font-mono text-indigo-400">{financialWeight}%</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between items-center mb-1 text-xs">
                ...
                <span className="font-mono text-indigo-400">{cyberWeight}%</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between items-center mb-1 text-xs">
                ...
                <span className="font-mono text-indigo-400">{weatherWeight}%</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between items-center mb-1 text-xs">
                ...
                <span className="font-mono text-indigo-400">{esgWeight}%</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div>
              <div className="flex justify-between items-center mb-1 text-xs">
                ...
                <span className="font-mono text-indigo-400">{operationalWeight}%</span>
              </div>
              <input
                type="range"
                min="0"
          ...
          <div className="pt-4 border-t border-slate-800/60 flex items-center justify-between">
            ...
            {canWrite ? (
              <button
                ...
                className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-xs font-bold text-white transition flex items-center gap-2 shadow-lg shadow-indigo-500/20 disabled:opacity-40 cursor-pointer"
              >
        ...
        <div className="space-y-6 font-sans">
          ...
          <div className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-5 space-y-3">
            <h3 className="text-sm font-semibold text-slate-200 font-display">The Active Risk Formula</h3>
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Where each variable is normalised as a risk deficit (e.g. <code className="font-mono bg-slate-900 px-1 py-0.5 rounded text-indigo-400 font-semibold">Fin = 100 - Health</code>). Dynamic calculation occurs instantly on the express server.
            </p>
          </div>

          {/* Country Level averages */}
          <div className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-slate-200 mb-4 flex items-center gap-1.5 font-display">
              ...
              {countryAverages.map((item) => (
                <div key={item.name} className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex-1 max-w-[120px] h-1.5 bg-slate-900 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-500" style={{ width: `${item.avg}%` }} />
                  </div>
